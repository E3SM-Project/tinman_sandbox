...
void gradient_sphere (const double* const field, const Derivative& deriv,
                      const double* const Dinv,  double* grad_field);
...
*/
  ...
  // More familiar name for number of elements/points/levels
  constexpr int np     = NUM_POINTS;
  constexpr int nlev   = NUM_LEVELS;
  constexpr int nelems = NUM_ELEMENTS;

  // Create local arrays
  double* grad_ps = new double[nelems*np*np*2];


  // Extract arrays from data (for notation shortness)
  double* elem_state_ps_v = data.arrays.elem_state_ps_v;
  double* elem_Dinv       = data.arrays.elem_Dinv;


  if (data.control.rsplit==0)
    gradient_sphere (elem_state_ps_v, data.deriv, elem_Dinv, grad_ps);
/*
  do ie=nets,nete
