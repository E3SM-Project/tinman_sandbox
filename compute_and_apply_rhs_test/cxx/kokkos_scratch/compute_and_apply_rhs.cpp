...
struct update_state {
  ...
  void compute_velocity(Kokkos::TeamPolicy<>::member_type &team,
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange(team, NUM_LEV),
      ...
      Kokkos::parallel_for(Kokkos::ThreadVectorRange(team, NP * NP * 2),
                           [&](const int idx) {
      ...
      Kokkos::parallel_for(Kokkos::ThreadVectorRange(team, NP * NP),
        ...
        const Real gpterm =
            m_region.T(ie, m_n0)(ilev, igp, jgp) / pressure(ilev, igp, jgp);

        const Real v1 = m_region.U(ie, m_n0)(ilev, igp, jgp);
        const Real v2 = m_region.V(ie, m_n0)(ilev, igp, jgp);

        Real glnps1 =
            PhysicalConstants::Rgas * gpterm * grad_p(ilev, 0, igp, jgp);
        vtens1(igp, jgp) =
            v_vadv(igp, jgp, 0) +
        ...
        Real glnps2 =
            PhysicalConstants::Rgas * gpterm * grad_p(ilev, 1, igp, jgp);
        vtens2(igp, jgp) =
            v_vadv(igp, jgp, 1) -
    ...
    // Pressure = f_P(CONSTANT)
    // Grad_P = \del Pressure
    // T_v = f_T(CONSTANT)
    // VTens1 = V_Vadv1 + v2 * (CONSTANT + Vort) - Ephi_grad1 - CONSTANT * T_v /
    // Pressure * Grad_P
    // U(i+1) = CONSTANT * (U(i) + dt ^ 2 * VTens1)
    team.team_barrier();
  }
  ...
  void compute_eta_dpdn(Kokkos::TeamPolicy<>::member_type &team) const {
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange(team, NUM_LEV_P),
                         [&](const int ilev) {
      ScratchView<Real[NP][NP]> eta_dot_dpdn_ie(team.team_scratch(0));
      Kokkos::parallel_for(Kokkos::ThreadVectorRange(team, NP * NP),
                           [&](const int idx) {
        const int igp = idx / NP;
        const int jgp = idx % NP;
        eta_dot_dpdn_ie(igp, jgp) = 0.0;
      });
      ...
      Kokkos::parallel_for(Kokkos::ThreadVectorRange(team, NP * NP),
        ...
        m_region.ETA_DPDN(ie)(ilev, igp, jgp) +=
            PhysicalConstants::eta_ave_w * eta_dot_dpdn_ie(igp, jgp);
      });
    });
    team.team_barrier();
  }
  ...
  void operator()(Kokkos::TeamPolicy<>::member_type team) const {
    ...
    if (ie < m_data.num_elems()) {
      ...
      Kokkos::parallel_for(Kokkos::TeamThreadRange(team, NUM_LEV),
        ...
        ScratchView<Real[NP][NP]> p_ilev =
            subview(pressure, ilev, ALL(), ALL());
        ScratchView<Real[2][NP][NP]> grad_p_ilev =
            subview(grad_p, ilev, ALL(), ALL(), ALL());
        gradient_sphere(team, p_ilev, m_data, m_region.DINV(ie), grad_p_ilev);
        ...
        Kokkos::parallel_for(Kokkos::ThreadVectorRange(team, NP * NP),
          ...
          vgrad_p(ilev, igp, jgp) =
              v1 * grad_p(ilev, 0, igp, jgp) + v2 * grad_p(ilev, 1, igp, jgp);
        ...
        vorticity_sphere(team, m_region.UN0(ie, ilev), m_region.VN0(ie, ilev),
                         m_data, m_region.METDET(ie), m_region.D(ie),
                         vort_ilev);
      });
      ...
      if (m_qn0 == -1) {
        ...
        ExecViewUnmanaged<Real[NUM_LEV][NP][NP]> qdp =
            m_region.QDP(ie, m_qn0, 1);
        Kokkos::parallel_for(Kokkos::TeamThreadRange(team, NUM_LEV),
                             [&](const int ilev) {
          Kokkos::parallel_for(Kokkos::ThreadVectorRange(team, NP * NP),
                               [&](const int idx) {
            ...
            const int jgp = idx % NP;
            Real Qt =
                qdp(ilev, igp, jgp) / m_region.DP3D(ie, m_n0)(ilev, igp, jgp);
            T_v(ilev, igp, jgp) = m_region.T(ie, m_n0)(ilev, igp, jgp) *
                                  (1.0 + (PhysicalConstants::Rwater_vapor /
      ...
      preq_hydrostatic(team, phis_ie, T_v, pressure, m_region.DP3D(ie, m_n0),
                       PhysicalConstants::Rgas, phi_ie);
          ...
          Kokkos::parallel_for(Kokkos::TeamThreadRange(team, NUM_LEV_P),
            ...
            Kokkos::parallel_for(Kokkos::ThreadVectorRange(team, NP * NP),
              ...
              ttens(ilev, igp, jgp) =
                  ...
                  kappa_star(ilev, igp, jgp) * T_v(ilev, igp, jgp) *
        ...
        Kokkos::parallel_for(Kokkos::TeamThreadRange(team, NUM_LEV),
          ...
          Kokkos::parallel_for(Kokkos::ThreadVectorRange(team, NP * NP),
            ...
            m_region.T(ie, m_np1)(ilev, igp, jgp) =
                m_region.SPHEREMP(ie)(igp, jgp) *
                (m_region.T(ie, m_nm1)(ilev, igp, jgp) +
                 m_dt2 * ttens(ilev, igp, jgp));
...
void preq_omega_ps(const Kokkos::TeamPolicy<>::member_type &team,
  ...
  Kokkos::parallel_for(Kokkos::TeamThreadRange(team, NP * NP),
    ...
    for (int ilev = 1; ilev < NUM_LEV - 1; ++ilev) {
      ...
      omega_p(ilev, igp, jgp) = vgrad_p(ilev, igp, jgp) / p(ilev, igp, jgp) -
    ...
    omega_p(NUM_LEV - 1, igp, jgp) =
        vgrad_p(NUM_LEV - 1, igp, jgp) / p(NUM_LEV - 1, igp, jgp) -
        ckl * suml[igp][jgp] - ckk * term;
  });
  team.team_barrier();
}
