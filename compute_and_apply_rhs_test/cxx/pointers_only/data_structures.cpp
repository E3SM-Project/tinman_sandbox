  ...
  for (int ie = 0; ie < num_elems; ++ie) {
    for (int ip = 0; ip < np; ++ip) {
      for (int jp = 0; jp < np; ++jp) {
        for(int k = 0; k < 2; k++) {
          for(int l = 0; l < 2; l++) {
            AT_5D(elem_D, ie, ip, jp, k, l, np, np, 2, 2) = init_map(x, n++);
          }
        }

        double detD = AT_5D(elem_D, ie, ip, jp, 0, 0, np, np, 2, 2) *
                          AT_5D(elem_D, ie, ip, jp, 1, 1, np, np, 2, 2) -
                      AT_5D(elem_D, ie, ip, jp, 0, 1, np, np, 2, 2) *
                          AT_5D(elem_D, ie, ip, jp, 1, 0, np, np, 2, 2);

        AT_5D(elem_Dinv, ie, ip, jp, 0, 0, np, np, 2, 2) =
            AT_5D(elem_D, ie, ip, jp, 1, 1, np, np, 2, 2) / detD;
        AT_5D(elem_Dinv, ie, ip, jp, 0, 1, np, np, 2, 2) =
            -AT_5D(elem_D, ie, ip, jp, 0, 1, np, np, 2, 2) / detD;
        AT_5D(elem_Dinv, ie, ip, jp, 1, 0, np, np, 2, 2) =
            -AT_5D(elem_D, ie, ip, jp, 1, 0, np, np, 2, 2) / detD;
        AT_5D(elem_Dinv, ie, ip, jp, 1, 1, np, np, 2, 2) =
            AT_5D(elem_D, ie, ip, jp, 0, 0, np, np, 2, 2) / detD;
        ...
        AT_3D(elem_rmetdet, ie, ip, jp, np, np) =
            1. / AT_3D(elem_metdet, ie, ip, jp, np, np);

        for (int il = 0; il < nlev; ++il) {
          AT_4D(elem_derived_omega_p, ie, il, ip, jp, nlev, np, np) =
              init_map(x, n++);
          AT_4D(elem_derived_pecnd, ie, il, ip, jp, nlev, np, np) =
              init_map(x, n++);
          AT_4D(elem_derived_phi, ie, il, ip, jp, nlev, np, np) =
              init_map(x, n++);
          AT_5D(elem_derived_vn0, ie, il, ip, jp, 0, nlev, np, np, 2) =
              init_map(x, n++);
          AT_5D(elem_derived_vn0, ie, il, ip, jp, 1, nlev, np, np, 2) =
              init_map(x, n++);

          for (int iq = 0; iq < qsize_d; ++iq) {
            for (int qni = 0; qni < 2; ++qni) {
              AT_6D(elem_state_Qdp, ie, il, iq, qni, ip, jp, nlev, qsize_d, 2,
                    np, np) = init_map(x, n++);
            }
          }

          for (int it = 0; it < timelevels; ++it) {
            AT_6D(elem_state_v, ie, it, il, ip, jp, 0, timelevels, nlev, np, np,
                  2) = init_map(x, n++);
            AT_6D(elem_state_v, ie, it, il, ip, jp, 1, timelevels, nlev, np, np,
                  2) = init_map(x, n++);
            AT_5D(elem_state_T, ie, it, il, ip, jp, timelevels, nlev, np, np) =
                init_map(x, n++);
            AT_5D(elem_state_dp3d, ie, it, il, ip, jp, timelevels, nlev, np,
                  np) = init_map(x, n++);
          }
        }

        for (int il = 0; il < nlevp; ++il) {
          AT_4D(elem_derived_eta_dot_dpdn, ie, il, ip, jp, nlevp, np, np) =
              init_map(x, n++);
        }
      }
    }
...
void Derivative::init_data() {
  real values[16] = { -3.0000000000000000,  -0.80901699437494745,
...
void TestData::cleanup_data() { arrays.cleanup_data(); }
