...
void gradient_sphere(const real *const scalar, const TestData &data, int ielem,
                     real *const ds) {
  ...
  for (int j = 0; j < np; ++j) {
    for (int l = 0; l < np; ++l) {
      dsdx = dsdy = 0;
      for (int i = 0; i < np; ++i) {
        dsdx += Dvv[i][l] * AT_2D(scalar, i, j, np);
        dsdy += Dvv[i][l] * AT_2D(scalar, j, i, np);
      }
...
void divergence_sphere(const real *const v, const TestData &data, int ielem,
                       real *const div) {
  typedef real Dvv_type[np][np];
  ...
  for (int igp = 0; igp < np; ++igp) {
    for (int jgp = 0; jgp < np; ++jgp) {
      gv[igp][jgp][0] =
          AT_2D(metdet, igp, jgp, np) * (AT_4D(Dinv, igp, jgp, 0, 0, np, 2, 2) *
                                             AT_3D(v, igp, jgp, 0, np, 2) +
                                         AT_4D(Dinv, igp, jgp, 0, 1, np, 2, 2) *
                                             AT_3D(v, igp, jgp, 1, np, 2));
      gv[igp][jgp][1] =
          AT_2D(metdet, igp, jgp, np) * (AT_4D(Dinv, igp, jgp, 1, 0, np, 2, 2) *
                                             AT_3D(v, igp, jgp, 0, np, 2) +
                                         AT_4D(Dinv, igp, jgp, 1, 1, np, 2, 2) *
                                             AT_3D(v, igp, jgp, 1, np, 2));
    }
  }
  ...
  for (int igp = 0; igp < np; ++igp) {
    for (int jgp = 0; jgp < np; ++jgp) {
      ...
      AT_2D(div, igp, jgp, np) =
          (dudx + dvdy) * AT_2D(rmetdet, igp, jgp, np) * rrearth;
    }
  }
}

void vorticity_sphere(const real *const v, const TestData &data, int ielem,
                      real *const vort) {
  typedef real Dvv_type[np][np];
  ...
  for (int igp = 0; igp < np; ++igp) {
    for (int jgp = 0; jgp < np; ++jgp) {
      vcov[igp][jgp][0] =
          AT_4D(D, igp, jgp, 0, 0, np, 2, 2) * AT_3D(v, igp, jgp, 0, np, 2) +
          AT_4D(D, igp, jgp, 1, 0, np, 2, 2) * AT_3D(v, igp, jgp, 1, np, 2);
      vcov[igp][jgp][1] =
          AT_4D(D, igp, jgp, 0, 1, np, 2, 2) * AT_3D(v, igp, jgp, 0, np, 2) +
  ...
  for (int igp = 0; igp < np; ++igp) {
    for (int jgp = 0; jgp < np; ++jgp) {
      ...
      AT_2D(vort, igp, jgp, np) =
          (dvdx - dudy) * AT_2D(rmetdet, igp, jgp, np) * rrearth;
    }
  }
}
